...
function wpv_shortcode_wpv_post_link($atts){
    ...
    if(!empty($post)){
		...
		// If WPML is enabled, $post_link should contain the right permalink for the current post in the current language
		// However, if using the id attribute, we might need to adjust it to the translated post for the given ID
		global $sitepress;
		if ( isset( $sitepress ) ){
			$id = url_to_postid($post_link);
			$new_id = icl_object_id( $id, get_post_type($id), false, $sitepress->get_current_language());
			$post_link = get_permalink($new_id);
		}
...
function wpv_shortcode_wpv_post_body($atts){
	...
	*
	* Note that we only modify that list if suppress_filters is true
    */
	...
	*
	* Note that we only need to restore it if it was changed: if suppress_filters is true
	* Also, tell WordPress to sort again the $GLOBALS['wp_filter']['the_content'] list again if it needs to apply it again
...
function wpv_shortcode_wpv_post_url($atts) {
    ...
    // If WPML is enabled, $out should contain the right permalink for the current post in the current language
	// However, if using the id attribute, we might need to adjust it to the translated post for the given ID
    global $sitepress;
    if ( isset( $sitepress ) ) {
        $id = url_to_postid($out);
        $new_id = icl_object_id( $id, get_post_type($id), false, $sitepress->get_current_language());
        $out = get_permalink($new_id);
    }
